...
def selectBuySell():
    outputCsv="/mnt/sde1/geth/nft_analyse_v1/data/buySell/csv/buysell.csv"
    f = open(outputCsv,'w')
    writer = csv.writer(f)
    ...
    for transactionHash,value0 in ethTransferMap.items():
        ...
        # 4. 第二个buyer如果没有支出则跳过
        if transferList[2] not in value0 or value0[transferList[2]]["outAction"]!=True:
            continue
